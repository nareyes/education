...
WITH (
    ...
    PARTITION (
        [tripDate] RANGE RIGHT FOR VALUES
...
-- If we need to find all the customers who traveled with IAC in the month of January, all we need to do is use a simple filter on tripDate, such as in the following example.
-- This will ensure that only the data in the below partitions are read and not a full table scan.
SELECT customerId
FROM dbo.TripTable 
WHERE tirpDate BETWEEN '20220101' AND '20220131';
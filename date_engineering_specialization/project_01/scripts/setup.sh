...
export ecr_uri="public.ecr.aws/deeplearning-ai/de-c1w2-jupyter:latest"
...
#newgrp docker
docker pull $ecr_uri
docker run -d -it -p 8888:8888 $ecr_uri
...
CONTAINER_ID=$(docker ps -a --filter "ancestor=$ecr_uri" --format "{{.ID}}")
